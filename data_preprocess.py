...
def extract_keywords(data):
    ...
    for job in data:
        for req in job['reqs']:
            ...
            for item in formatted_tokens:
                if item == "$":
                    if current_group:
                        keyword_list.append(" ".join(current_group))
                        current_group = []
                else:
    ...
    tagged_keyword_list = pos_tag(keyword_list)
    keyword_list = [word for word, tag in tagged_keyword_list if tag != 'VB' and word in technology_keywords ]
